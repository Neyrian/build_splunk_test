...
if [ $# -eq 0 ]; then
    ...
    if ! [ -e $apache_ta_path ]; then
        log_message "WARN" "Apache TA doesn't exists. Download it from https://splunkbase.splunk.com/ and put the archive in $working_dir"
    fi
    if ! [ -e $windows_ta_path ]; then
        log_message "WARN" "Windows TA doesn't exists. Download it from https://splunkbase.splunk.com/ and put the archive in $working_dir"
    fi
    if ! [ -e $linux_ta_path ]; then
        log_message "WARN" "Linux TA doesn't exists. Download it from https://splunkbase.splunk.com/ and put the archive in $working_dir"
    fi
